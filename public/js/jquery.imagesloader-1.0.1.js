...
// #endregion
  ...
  + function ($) {
    ...
    ImagesLoader.prototype.btnRemoveClick = function (evt, obj) {
      ...
      var $element = $(element);
      var options = self.options;
      var imagesloaderRemain = self.imagesToLoad;

      // Find div and id image
      var $div = $(obj).closest('[data-type="image"]');

      
      self.imagesToLoad = jQuery.grep(imagesloaderRemain, function(value) {
        if(value.Name != $div.data('id')){
          return value
        }
      });

      console.log(self.imagesToLoad)
    ...
    ImagesLoader.prototype.btnMainClick = function (evt, obj) {
      ...
      idx = self.imagesToLoad.map(function (image) { return image.Name; }).indexOf(id);

      if (idx !== -1) {
        self.imagesToLoad[0] = self.imagesToLoad[idx];
        self.imagesToLoad[idx] = main;
      }
      console.log(self.imagesToLoad)
    };
